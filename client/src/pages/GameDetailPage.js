...
import { ArrowLeft, Monitor, Users, Clock, Zap, ExternalLink, Search, Calendar, Code2, ChevronLeft, ChevronRight } from 'lucide-react';
...
function Screenshots({ gameId }) {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/api/games/${gameId}/screenshots`)
      .then(r => r.json())
      .then(data => { setScreenshots(Array.isArray(data) ? data : []); setLoading(false); })
  ...
  if (loading) return (
    <div style={{ marginTop:'1.5rem' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'0.5rem' }}>
        {[1,2,3].map(i => <div key={i} style={{ aspectRatio:'16/9', background:'var(--surface2)', borderRadius:'var(--r-sm)', animation:'pulse 1.5s infinite' }}/>)}
      </div>
    </div>
  );
  ...
  return (
    <div style={{ marginTop:'1.5rem' }}>
      <div style={{ fontSize:'0.7rem', fontWeight:700, letterSpacing:'0.08em', textTransform:'uppercase', color:'var(--text-3)', marginBottom:'0.75rem' }}>
        Screenshots
      </div>
      ...
      <div style={{ position:'relative', aspectRatio:'16/9', borderRadius:'var(--r-lg)', overflow:'hidden', background:'var(--surface2)', marginBottom:'0.5rem', cursor:'pointer' }} onClick={() => setLightbox(current)}>
        <img src={screenshots[current]} alt={`Screenshot ${current+1}`} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e => e.target.style.display='none'}/>
        {screenshots.length > 1 && (
          <>
            <button onClick={e => { e.stopPropagation(); setCurrent(c => (c-1+screenshots.length)%screenshots.length); }}
            ...
            <button onClick={e => { e.stopPropagation(); setCurrent(c => (c+1)%screenshots.length); }}
      ...
      {/* Thumbnails */}
      <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.min(screenshots.length,6)},1fr)`, gap:'0.4rem' }}>
        {screenshots.map((src, i) => (
          <div key={i} onClick={() => setCurrent(i)} style={{ aspectRatio:'16/9', borderRadius:'var(--r-sm)', overflow:'hidden', cursor:'pointer', border: i===current ? '2px solid var(--primary)' : '2px solid transparent', opacity: i===current ? 1 : 0.65, transition:'all 0.15s' }}>
            <img src={src} alt={`Thumb ${i+1}`} style={{ width:'100%', height:'100%', objectFit:'cover' }} onError={e => e.target.parentElement.style.display='none'}/>
          </div>
        ))}
      </div>
      ...
      {/* Lightbox */}
      {lightbox !== null && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.92)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'2rem' }} onClick={() => setLightbox(null)}>
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l-1+screenshots.length)%screenshots.length); }}
          ...
          <img src={screenshots[lightbox]} alt="" style={{ maxWidth:'90vw', maxHeight:'85vh', objectFit:'contain', borderRadius:'var(--r)' }} onClick={e => e.stopPropagation()}/>
          <button onClick={e => { e.stopPropagation(); setLightbox(l => (l+1)%screenshots.length); }}
